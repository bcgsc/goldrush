...
static inline void
assert_eof(std::istream& in, const std::string& path)
{
  if (!in.eof()) {
    in.clear();
    std::string s;
...
operator>>(std::istream& in, expect o)
{
  for (const char* p = o.s; *p != '\0'; ++p) {
    if (*p == ' ') {
      ...
      char c = in.get();
      if (!in || c != *p) {
        std::cerr << "error: Expected `" << p << "' and saw ";
        if (in)
          std::cerr << '`' << c << "'\n";
        else if (in.eof())
...
static inline std::istream&
operator>>(std::istream& in, Ignore o)
{
...
static inline void
readFile(const char* path, Vector& s)
{
