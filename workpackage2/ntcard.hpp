...
inline void
stRead(const string& seq, std::vector<std::vector<uint16_t>>& t_Counters, std::vector<size_t>& totKmer_vec)
{
...
getHist(const vector<string>& inFiles, const unsigned kLen, const unsigned nThr, const unsigned covMax, std::vector<std::string> seed_vec = std::vector<std::string>(0))
{
#ifdef _OPENMP
    ...
    std::vector<std::vector<size_t>> histArray_vec(num_histArray, std::vector<size_t>(covMax + 2, 0));
	...
	std::vector<std::vector<uint16_t>> t_Counters(num_histArray, std::vector<uint16_t>(nts::nSamp * nts::rBuck, 0));
...
//#pragma omp parallel for schedule(dynamic)
    for (unsigned file_i = 0; file_i < inFiles.size(); ++file_i) {
        ...
        btllib::SeqReader reader(inFiles[file_i], btllib::SeqReader::Flag::LONG_MODE);
#pragma omp parallel
...
for (size_t curr_histArray = 0; curr_histArray < num_histArray; ++curr_histArray){
