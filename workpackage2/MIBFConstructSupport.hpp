...
public:
	...
	MIBFConstructSupport(size_t expectedEntries, unsigned k,
			unsigned numHashFunction, double occupancy, size_t filterSize,
			const vector<string> &spacedSeeds = vector<string>(0)) :
			m_isBVMade(false), m_isMIBFMade(false), m_expectedEntries(
					expectedEntries), m_k(k), m_h(numHashFunction), m_occupancy(
					occupancy), m_spacedSeeds(spacedSeeds), m_counts(
					vector<T>()), m_filterSize(filterSize) {
		std::cerr << "m_filterSize: " << m_filterSize << std::endl;
		m_bv = sdsl::bit_vector(m_filterSize);
	}
	...
	void insertBV(H &itr, std::vector<std::unique_ptr<btllib::BloomFilter>>& solid_vec) {
		//assert(!m_isBVMade);
		/* init rolling hash state and compute hash values for first k-mer */
		for (; itr != itr.end(); ++itr) {
			for (unsigned i = 0; i < m_h; ++i) {
	...
	*/
	...
	MIBloomFilter<T> *getEmptyMIBF() {
		MIBloomFilter<T> *miBF = new MIBloomFilter<T>(m_h, m_k, m_bv_il, m_rankSupport,
				m_spacedSeeds);
	...
	void insertMIBF(MIBloomFilter<T> &miBF, const std::vector<uint64_t>& hash_vec, T id) {
		//assert(m_isBVMade & !m_isMIBFMade);
		//get positions
#if _OPENMP
#pragma omp parallel for
		...
		for (size_t i = 0; i < hash_vec.size(); ++i ) {
			...
			T count = __sync_add_and_fetch(&m_counts[rank], 1);
			T randomNum = std::hash<T> { }(randomSeed) % count;
			//std::cerr << "id: " << id << " randomNum: " << randomNum << " randomSeed: " << randomSeed <<std::endl;
			if (randomNum == count - 1) {
				miBF.setData(rank, id);
			}
		}
	...
	void insertMIBF(MIBloomFilter<T> &miBF, H &itr, T id, std::vector<std::unique_ptr<btllib::BloomFilter>>& solid_vec) {
		//get positions
		hashSet values;
		values.set_empty_key(miBF.size());
		while (itr != itr.end()) {
